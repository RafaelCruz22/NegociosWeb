<?php
  ...
  if(isset($_GET["action"])){
    switch($_GET["action"]){
      ...
      case "del":
        $mode = "delete";
         $strsql = "delete * from productos where prdid = %d;";
        $strsql = sprintf($strsql,$_GET["prdid"]);
        $producto = deleteRegistro($strsql);
        if($producto){
          $modeDescription[$mode] .= " " . $_GET["prdid"];
        }else{
          redirectWithMessage('EL producto no existe!', 'ej8.php');
        }
    ...
    <form action="ej9.php?action=<?php echo $_GET["action"]; ?>" method="POST">
        <?php
         ...
         <input type="submit" name="btnInsert" id="btnInsert" value="Guardar" />
          <input type="submit" name="btndel" id="btndel" value="borrar" />
    </form>
  </body>
</html>